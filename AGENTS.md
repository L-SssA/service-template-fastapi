...
- ❌ **实现总结/实施报告** - 如 `CELERY_BEAT_IMPLEMENTATION.md`
- ❌ **快速参考卡片** - 如 `CHEATSHEET.md`
- ❌ **变更记录/更新日志** - 如 `CHANGELOG.md`, `UPDATE_LOG.md`
- ❌ **重构说明** - 如 `REFACTOR_SUMMARY.md`
- ❌ **任务交付清单** - 如 `DELIVERY_CHECKLIST.md`
- ❌ **版本发布说明** - 如 `RELEASE_NOTES.md`
- ❌ **其他临时性/过渡性文档**

**正确做法**：

- ✅ 所有技术细节直接整合到现有文档（`README.md` 或 `docs/` 中的相关文档）
- ✅ 实现细节应该在代码注释中体现
- ✅ 变更内容直接更新到相关章节，不单独成文
...
**原子性原则**：当移动代码（模型、路由、任务等）到新位置时，必须保证：

1. ✅ 在新位置创建完整的代码定义
2. ✅ **立即从原位置删除已迁移的代码**（避免重复定义）
3. ✅ 更新所有相关的导入语句
4. ✅ 同步更新文档说明新的目录结构
...
- **最小化原则**：只维护对项目运行和维护必不可少的文档
- **禁止临时文档**：任何情况下都不得创建临时性、总结性、记录性的文档
...
- [ ] **所有修改是否直接更新到了现有文档中？**
- [ ] **是否未创建实现总结、快速参考卡片、交付清单等禁止类型的文档？**
- [ ] **如创建了新的 .md 文件，是否确属必要且无法整合到现有文档中？**
...
1. **创建新文件** → 在新位置创建完整的代码定义
2. **删除旧代码** → **立即从原位置删除已迁移的代码**（关键步骤！）
3. **更新导入** → 更新所有引用该代码的导入语句
4. **更新文档** → 更新 `README.md` 和 `AGENTS.md` 中的目录结构说明
5. **验证测试** → 确保所有功能正常工作
...
```bash
# 错误做法：只创建新文件，不删除旧代码 ❌
# 正确做法：创建 app/models/system.py 后，立即删除 celery.py 中的 CeleryStatusData ✅
```
